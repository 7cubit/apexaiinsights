...
func SetupChatEndpoint(app *fiber.App, repo *Repository) {
	...
	if apiKey == "" {
		log.Println("Warning: OPENAI_API_KEY not set. Chat features will trigger errors.")
	}
	...
	app.Post("/v1/ask", func(c *fiber.Ctx) error {
		...
		// For now, we'll get a quick summary count
		summaryStats, _ := repo.RunReadOnlyQuery(`
			SELECT count(*) as sessions, MAX(created_at) as last_event 
			FROM wp_apex_events 
			WHERE created_at > DATE_SUB(NOW(), INTERVAL 24 HOUR)
		`)
		contextStr := "No recent data."
		if len(summaryStats) > 0 {
			contextStr = fmt.Sprintf("Recent (24h) Sessions: %v", summaryStats[0]["sessions"])
		}
		...
		if err != nil {
			...
			return c.Status(fiber.StatusInternalServerError).JSON(fiber.Map{
				"error": fmt.Sprintf("Failed to generate insight: %v", err),
			})
		}
