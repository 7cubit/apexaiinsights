...
import { Shield, Lock, AlertTriangle, Search, Activity, Trash2 } from 'lucide-react';
...
export default function SecurityCenter() {
    ...
    const fetchLogs = async () => {
        try {
            // In real dev, point to http://localhost:8080 or use vite proxy
            const res = await fetch('http://localhost:8080/v1/security/audit');
            if (res.ok) {
    ...
    const handleScan = async () => {
        ...
        setScanResult(null);
        try {
            const res = await fetch(`http://localhost:8080/v1/security/scan?url=${encodeURIComponent(scanUrl)}`);
            const data = await res.json();
            setScanResult(data);
        } catch (e) {
    ...
    const handleDeletionRequest = async () => {
        ...
        try {
            await fetch(`http://localhost:8080/v1/compliance/delete?email=${encodeURIComponent(deletionEmail)}`, { method: 'POST' });
                    ...
                    <div className="space-y-4">
                        <div className="flex gap-2">
                            ...
                            <button
                                onClick={handleScan}
                                disabled={loading}
                                className="px-4 py-2 bg-red-500 hover:bg-red-600 text-white rounded-lg font-medium transition-all disabled:opacity-50"
                            >
